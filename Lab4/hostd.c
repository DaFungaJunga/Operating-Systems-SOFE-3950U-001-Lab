...
int main(int argc, char *argv[])
{
	...
	n.address= alloc_mem(n.memory,n.priority,n.resourceType);
	if (n.address==8080){
		printf("no memory\n");
		exit(0);
	}
	while(n.runtime>0){
    	...
    	if (pid==0){
			...
			n.runtime=0;
			free_mem(n.address,n.memory,n.resourceType);
	...
	while(termination[1]>0){
		...
		m.address= alloc_mem(m.memory,m.priority,m.resourceType);
		if (m.address==8080){
		...
		if (pid==0){
			execl("/home/dafungajunga/Documents/ostut8/process","process",NULL);
				...
				if (m.runtime==1){
					...
					free_mem(m.address,m.memory,m.resourceType);
					termination[1]-=1;
					printf("done\n");
					continue;
				}
				...
				if (m.runtime>0){
					...
					if(m.runtime>0){
						...
						m.runtime-=1;
						waitpid (m.pid, &status, 0);
						free_mem(m.address,m.memory,n.resourceType);
						termination[1]-=1;
						printf("done\n");
						continue;
					}	
